...
export class ImportOrdersComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const orderUploadUrl: string = LoopBackConfig.getPath() + '/' + LoopBackConfig.getApiVersion() +
  ...
  generateStockOrderVend() {
    if (this.uploader.queue.length) {
      ...
      this.loading = true;
      const deliverFromStore = this.stores.find(x => x.objectId === this.selectedWarehouseId);
