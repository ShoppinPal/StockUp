                ...
                if (err1.code === 'ENOENT') {
                    ...
                    Container.createContainer({name: userId}, function (err2, container2) {
                        if (err2) {
                            logger.debug({
                                log: {
                                    message: 'Container > beforeRemote > upload > Could not create a new container > unexpected error',
                                    error: err2
                                }
                            });
                            console.error(err2);
                    ...
                    logger.debug({
                        log: {
                            message: 'Container > beforeRemote > upload > Container does not exist > unexpected error',
                            error: err1
                        }
                    });
                    console.error(err1);
                                        ...
                                        return createReportModelForExcelWithoutSupplier(singleOrder, Container, next)
                                            ...
                                            .catch(function (error) {
                                                if (error instanceof Error) {
                                                    ...
                                                    logger.error({
                                                        err: error,
                                                        message: 'Container > afterRemote > upload > end_parsed'
                                                    });
                                                }
                                                else {
                                                    // log.error('Container > afterRemote > upload > end_parsed',
                                                                            ...
                                                                            .spread(function (reportModelInstance, storeModelInstance, storeConfigInstance) {
                                                                                ...
                                                                                return ReportModel.sendPayload(reportModelInstance, options, queueUrl, next)
                                                                                    .then(function (updatedReportModelInstance) {
                                                                                        logger.trace({
                                                                                            log: {
                                                                                                message: 'updatedReportModelInstance:',
