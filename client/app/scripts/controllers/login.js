...
angular.module('ShoppinPalApp')
  .controller('LoginCtrl', [
    '$scope', '$sessionStorage', '$state', '$cookies', /* angular's modules/services/factories etc. */
    'UserModel', /* shoppinpal's custom modules/services/factories etc. */
    function ($scope, $sessionStorage, $state, $cookies,
              UserModel) {
      $scope.userName = '';
      $scope.password = '';
      $scope.errors = {
      ...
      $scope.login = function login(username, password) {
        ...
        $scope.waitOnPromise = UserModel.login({rememberMe: true}, {
          ...
          .$promise.then(function (accessToken) {
              //for warehouse-v2 app to access these by default
              document.cookie = '$LoopBackSDK$created='+accessToken.created;
              document.cookie = '$LoopBackSDK$id='+accessToken.id;
              document.cookie = '$LoopBackSDK$rememberMe='+accessToken.rememberMe;
              document.cookie = '$LoopBackSDK$ttl='+accessToken.ttl;
              document.cookie = '$LoopBackSDK$user='+accessToken.user;
              document.cookie = '$LoopBackSDK$userId='+accessToken.userId;
