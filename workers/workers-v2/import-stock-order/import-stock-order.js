    ...
    return utils.savePayloadConfigToFiles(payload)
      ...
      .then(function (dbInstance) {
        ...
        if (payload.reportId === undefined || payload.reportId === null) {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Report not found in payload' });
          return Promise.reject('Report not found');
        }
        else {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Will look for skus included in the report' });
      ...
      .then(function (lineItems) {
        ...
        if (lineItems.length>0) { // if rows already exist, it means the raw data was imported already
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Will look for products in db against the SKUs', skus: _.pluck(lineItems, 'sku') });
          ...
          logger.debug({ messageId: messageId, commandName: commandName, message: 'No line items found for the report, will create line items based on reorder_point' });
          return Promise.reject(commandName + ' > raw data has not been imported yet');
        }
      })
      .then(function (response) {
        productInstances = response;
        logger.debug({ messageId: messageId, commandName: commandName, message: `Found ${productInstances.length} products against ${lineItemInstances.length} line items` });
        if (productInstances.length>lineItemInstances.length) {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'There are duplicate SKUs in the database, will ignore them' });
        }
        else if (productInstances.length<lineItemInstances.length) {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Some SKUs were not found, will mark them as unboxed' });
        }
      ...
      .then(function (response) {
        ...
        logger.debug({ messageId: messageId, commandName: commandName, message: `Found ${inventoryInstances.length} inventories against ${lineItemInstances.length} line items` });
        if (productInstances.length !== inventoryInstances.length) {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Could not find all matching inventories, but imported orders do not care about inventory for now' });
        }
        return Promise.map(lineItemInstances, function (eachLineItem) {
          ...
          logger.debug({ messageId: messageId, commandName: commandName, message: `Updating productId and inventoryId for ${eachLineItem.sku}` });
      ...
      .then(function (response) {
        logger.debug({ messageId: messageId, commandName: commandName, message: 'Done updating the stock order line item models with required product and inventory info', result: response.result });
        logger.debug({ messageId: messageId, commandName: commandName, message: 'Will change the status of report to warehouse fulfilment' });
