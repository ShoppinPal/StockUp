    ...
    try {
      ...
      return utils.savePayloadConfigToFiles(payload)
        ...
        .then(function (fetchedProducts) {
          ...
          logger.debug({ messageId: messageId, commandName: commandName, message: `Found ${incrementalProducts.length} new products, will filter only required data from them` });
          logger.debug({ messageId: messageId, commandName: commandName, message: `Found ${productsToDelete.length} deleted products, will delete them from the database` });
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Will go on to look for vend tags' });
          var argsForTags = vendSdk.args.tags.fetch();
          return vendSdk.tags.fetchAll(argsForTags, vendConnectionInfo);
        })
        .then(function (tags) {
          logger.debug({ messageId: messageId, commandName: commandName, message: `Found ${tags.length} tags, will attach them to their products` });
          vendTags = tags;
        ...
        .then(function (dbInstance) {
          ...
          db = dbInstance;
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Will look for suppliers to attach to products' });
        ...
        .then(function (supplierModelInstances) {
          logger.debug({ messageId: messageId, commandName: commandName, message: `Found ${supplierModelInstances.length} suppliers, will attach them to products` });
          ...
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Attached suppliers and filtered product objects, will download them into database' });
        ...
        .then(function (response) {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Saved new version number for products', result: response ? response.result || response : '' });
          return Promise.resolve();
        })
        .catch(function (error) {
          if (error === 'noIncrementalProducts') {
            logger.error({ messageId: messageId, commandName: commandName, message: 'No new products found, will exit worker' });
            if (db) {
              return db.collection('SyncModel').updateOne({
                  $and: [
                    {
