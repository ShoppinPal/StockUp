...
var runMe = function (payload, config, taskId, messageId) {
    ...
    try {
        ...
        logger.debug({
            messageId: messageId,
            commandName: commandName,
            message: `This worker will generate stock order for outlet ${payload.outletName} and supplier ${payload.supplierName}`
        });
        return utils.savePayloadConfigToFiles(payload)
            ...
            .then(function (dbInstance) {
                ...
                if (payload.reportId === undefined || payload.reportId === null) {
                    logger.debug({
                        messageId: messageId,
                        commandName: commandName,
                        message: `Report does not exist, will create a new one for outlet ${payload.outletName}`
                    });
            ...
            .then(function (response) {
                logger.debug({
                    messageId: messageId,
                    commandName: commandName,
                    message: `Will look for products belonging to supplier ID ${payload.supplierId}`
                });
            ...
            .then(function (supplierProducts) {
                ...
                logger.debug({
                    messageId: messageId,
                    commandName: commandName,
                    message: `Found ${supplierProducts.length} products belonging to supplier ID ${payload.supplierId}`
                });
                logger.debug({
                    messageId: messageId,
                    commandName: commandName,
                    message: `Will look for their inventories for outlet id ${payload.outletId}`
                });
            ...
            .then(function (response) {
                ...
                logger.debug({
                    messageId: messageId,
                    commandName: commandName,
                    message: 'Will change the status of report to manager new order'
                });
