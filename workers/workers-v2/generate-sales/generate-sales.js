var runMe = function (payload, config, taskId, messageId) {
  ...
  try {
    ...
    try {
      ...
      logger.debug({ messageId: messageId, commandName: commandName, message: `This worker will create a sales from warehouse ${payload.warehouseOutletName} to the outlet ${payload.customerOutletName}` });
      return utils.savePayloadConfigToFiles(payload)
        ...
        .then(function (warehouseOutlet) {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Found the warehouse outlet', data: warehouseOutlet.data });
        ...
        .then(function (report) {
          if (report === null) {
            ...
            logger.debug({ messageId: messageId, commandName: commandName, message: 'Found report', report: report });
            logger.debug({ messageId: messageId, commandName: commandName, message: 'Will go on to fetch items included in this report' });
        ...
        .spread(function (productsInReport, report) {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Found these items belonging to the report', productsInReport: productsInReport });
          if (productsInReport.length !== report.totalRows) {
            ...
            logger.debug({ messageId: messageId, commandName: commandName, message: 'Will create sales for the items actually found' });
          }
          var totalAmount = 0;
          ...
          logger.debug({ messageId: messageId, commandName: commandName, message: `Finding register against the current warehouse outlet ID ${payload.warehouseOutletId}` });
        ...
        .then(function (data) {
          if (!data.registers.length) {
            logger.debug({ messageId: messageId, commandName: commandName, message: 'Could not find any registers belonging to this organisation' });
          ...
          if (registerFound) {
            ...
            logger.debug({ messageId: messageId, commandName: commandName, message: `Found the register ID ${salesPayload.register_id}` });
        ...
        .then(function () {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Creating a sales with following data', salesPayload: salesPayload });
          return vendSdk.sales.create(salesPayload, vendConnectionInfo);
        })
        .then(function (saleCreated) {
          logger.debug({ messageId: messageId, commandName: commandName, message: 'Created a sale', saleCreated: saleCreated });
