      ...
      return utils.savePayloadConfigToFiles(payload)
        .then(function () {
          try {
            ...
            logger.tag('datasourcesFile').debug({
                messageId: messageId,
                commandName: commandName,
            ...
            return Promise.resolve()
              .then(function () { // (1) create a report if payload.reportId is empty
                ...
                logger.tag('Creating Report').debug({ messageId: messageId, message: 'Starting...' });
                if (payload.reportId === undefined || payload.reportId === null) {
                  ...
                  return client.models.UserModel.loginAsync(payload.credentials) // get an access token
                    .then(function (token) {
                      logger.debug({ messageId: messageId, message: `Logged in as ${payload.credentials.email}` });
                    ...
                    return stockOrderLineitemModels.countAsync()
                      .then(function (count) {
                        ...
                        if (count>0) { // if rows already exist, it means the raw data was imported already
                          //console.log('[MessageId : '+messageId+']'+'Will run the OP for: importStockOrderCached');
                          logger.tag('importStockOrderCached').debug({
                              messageId: messageId,
                              message: 'Will run the OP for: importStockOrderCached'
                          });
              ...
              .tap(function importStockOrderCached(methodName) {
                if (methodName !== 'importStockOrderCached') {
                  //console.log('[MessageId : '+messageId+']'+'will skip importStockOrderCached');
                  logger.debug({ messageId: messageId, message: 'will skip importStockOrderCached' });
                  return Promise.resolve();
                }
                else {
                          ...
                          return stockOrderLineitemModels.countAsync()
                            .then(function (count) {
                              ...
                              logger.tag('PaginationInfo').debug({
                                  messageId: messageId,
                                  message: `Will traverse ${count} rows by fetching ${totalPages} of size <= ${pageSize}`
                              });
                                      ...
                                      return Promise.map(
                                          ...
                                          return cache.getAsync(key)
                                            .then(function (response) {
                                              try {
                                                ...
                                                logger.tag('Memcache').error({
                                                  err: err,
                                                  message: 'Looks like Memcache returned invalid json data. Check response object in this log object',
                                                  response: response
                                                });
                                              }
                                              return Promise.resolve();
                                            ...
                                            .catch(function (error) {
                                              //console.error('[MessageId : '+messageId+']'+'failed to lookup vend data from cache, maybe it expired or maybe it was never placed there');

                                              itemsFailedFromIronCache.push(lineitem.sku);
                                              // console.log('[MessageId : '+messageId+']'+commandName,'Pushed failed item : ',lineitem);
                                              // console.log('[MessageId : '+messageId+']'+commandName, 'ERROR', error.stack);
                                              // console.log('[MessageId : '+messageId+']'+'ignoring this ERROR, so that we may finish the rest of the process');
                                              logger.error({
                                                err: error,
                                                messageId: messageId,
                                                commandName: commandName,
                                                message: 'Push failed. Check lineitem property in this log obj. Ignoring this error so that we may finish the rest of the process'
                                              });
                                              return Promise.resolve();
                                            });
                                        },
                                        ...
                                        .then(function () {
                                          ...
                                          return Promise.map(lineitems,
                                                ...
                                                return vendSdk.products.fetchBySku({sku: {value: lineitem.sku}}, connectionInfo)
                                                  ...
                                                  .catch(function (error) {
                                                    ...
                                                    logger.error({
                                                      ...
                                                      messageId: messageId,
                                                      commandName: commandName,
                                                      message: 'Vend SKU lookup failed',
                                                      sku: lineitem.sku
                                                    });
                                                    return Promise.resolve();
                                                  });
                                            ...
                                            .then(function () {
                                              // console.log('[MessageId : '+messageId+']'+'cross-referenced and filled out lineitems against data from ElastiCache');
                                              // console.log('[MessageId : '+messageId+']'+'will send update(s) to loopback');
                                              // console.log('[MessageId : '+messageId+']'+"Updating lineitems with : ",lineitems);
                                              logger.debug({
                                                  messageId: messageId,
                                                  message: 'cross-referenced and filled out lineitems against data from ElastiCache. will send update(s) to loopback. Updating lineitems',
                                                  lineitems: lineitems
                                              });
