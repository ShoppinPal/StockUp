...
docker push gcr.io/warehouse-build/warehouse:production-${_BUILD_TAG}
fi
if [ "${_BRANCH_NAME}" == "develop" ]; then
scripts/build/tag ${_BUILD_TAG} "dev-${_BUILD_TAG}"
docker push gcr.io/warehouse-build/warehouse:dev-${_BUILD_TAG}
fi
if [[ "${_BRANCH_NAME}" =~ ^release.* ]]; then
scripts/build/tag ${_BUILD_TAG} "staging-${_BUILD_TAG}"
docker push gcr.io/warehouse-build/warehouse:staging-${_BUILD_TAG}
fi
